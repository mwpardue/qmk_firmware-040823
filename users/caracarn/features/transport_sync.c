...
void user_transport_update(void) {
    if (is_keyboard_master()) {
        rgb_split_config.rgb_matrix_ledmap_active = get_ledmap_active();
        user_state.rgb_matrix_ledmap_active = rgb_split_config.rgb_matrix_ledmap_active;
        transport_user_state = user_state.raw;
        dprintf("MASTER rgb_split_config.rgb_matrix_ledmap_active = %d\n", rgb_split_config.rgb_matrix_ledmap_active);
        ...
        dprintf("SLAVE rgb_split_config.rgb_matrix_ledmap_active = %d\n", rgb_split_config.rgb_matrix_ledmap_active);
    }
}
...
void user_transport_sync(void) {
    if (is_keyboard_master()) {
        ...
        static uint32_t last_sync[4], last_user_state = 0;
